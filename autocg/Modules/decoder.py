...
class RNNAttentionDecoder(nn.Module):
    ...
    def forward(self, input_seq, prev_output, context, context_mask, sem_z, hidden=None):
        ...
        for emb in embeddings.split(1, dim=1):
            ...
            output, att = self.attention(output, context, context_mask)
            att_w.append(att)
            if self.use_last_output:
            ...
            scores.append(score)
        return torch.stack(scores), hidden, output, torch.stack(att_w)
