...
class Decode(nn.Module):
    def __init__(self, vocab_size, emb_size, embedding, hidden_size, layers=1, dropout=0.0,
        ...
        self.pgen = nn.Linear(self.hidden_size * self.layers + self.hidden_size + self.emb_size, 1)
    ...
    def forward(self, input_seq, prev_output, context, context_mask,
                sent_enc_outputs, sent_mask, hidden=None):
        ...
        for emb in embeddings.split(1, dim=1):
            if self.input_feeding:
                input = torch.cat([emb.squeeze(1), prev_output], 1)
            else:
                input = emb.squeeze(1)
            ...
            output, sent_attn, sent_ctx = self.sent_attention(tree_output, sent_enc_outputs, sent_mask)
            # sent_ctx, sent_attn = self.sent_attention(tree_output, sent_enc_outputs, sent_mask)
            sent_att_w.append(sent_attn)

            # if self.use_content:
            #    con_attn_output, con_attn, con_ctx = self.content_attention(tree_output, content_outputs, content_mask)
            # con_ctx, con_attn = self.content_attention(tree_output, content_outputs, content_mask)
            # output = self.gelu(self.ffn(output + con_attn_output))
            # output = self.relu(self.ffn(output + con_attn_output + tree_output))
            # output = self.relu(self.ffn(output + con_attn_output))
            # new add
            # output = output + tree_output

            # output = self.relu(self.ffn(sent_ctx + con_ctx + tree_output))
            # output = self.tanh(self.ffn(sent_ctx + con_ctx + tree_output))
            # output = self.cat_ffn(torch.cat((output, con_attn_output), dim=1))

            # else:
            #    print("Error!, Now don't support to use other infomation!!!")
            #    exit()
