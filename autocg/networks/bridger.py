...
from autocg.utils.nn_funcs import positional_encodings_from_range
...
class MLPBridger(nn.Module):
    ...
    def forward(self, input_tensor):
        """
        Args:
            input_tensor: [layers,batch,hidden_size]
        """
...
class MultiTensorFusion(nn.Module):
    def __init__(self, encoder_dim, encoder_layer, decoder_dim, decoder_layer, mapping_type='mapping'):
        super(MultiTensorFusion, self).__init__()
        ...
        self.decoder_dim = decoder_dim
        self.decoder_layer = decoder_layer
        self.input_dim = encoder_dim * 2 # the last 2 represents to concatenate two tensor .
        self.output_dim = decoder_dim  # 2 represents the bi-direction .

        if self.mapping_type == 'link':  # concate opration .
            self.mapper = reflect()
        elif self.mapping_type == 'mapping':
            self.mapper = nn.Linear(self.input_dim, self.output_dim)

    def forward(self, *input):
        encoder = input[0]
        label = input[1]
        batch_size = encoder.size(1)
        # label_repeat = label.repeat(1, self.decoder_layer, 1)
        encoder_reset = encoder.permute(1, 0, 2).contiguous().view(batch_size, self.decoder_layer, -1)
        label_reset = label.permute(1, 0, 2).contiguous().view(batch_size, self.decoder_layer, -1)
        # perm_idx = torch.LongTensor([0,1,4,2,3,5])
        encoder_label = torch.cat([encoder_reset, label_reset], dim=2)
        output_tensor = self.mapper(encoder_label.view(batch_size * self.encoder_layer, -1)).view(batch_size,
                                                                                                  self.decoder_layer,
                                                                                                  -1)
        return output_tensor.permute(1, 0, 2).contiguous()
